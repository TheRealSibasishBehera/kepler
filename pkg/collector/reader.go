...
var (
	...
	currNodeEnergy = &CurrNodeEnergy{} // for node
...
func (c *Collector) reader() {
	...
	// sets and starts a new ticker (something like loops in a given interval)
	//containing a channel that will send the current time on the channel after each tick
	ticker := time.NewTicker(samplePeriod)

	//starting a go routine on a anonymous func call
	go func() {
		...
		//starts acpi powermeter
		acpiPowerMeter.Run()

		// starts a infinte loop
		for {
			select {
			case <-ticker.C:
				...
				lock.Lock()
				// A cgroup Time is Declared containing
				//type CgroupTime struct {
				//	CGroupPID      uint64
				//	PID            uint64
				//	ProcessRunTime uint64
				...
				//iterateble over collectors Table
				for it := c.modules.Table.Iter(); it.Next(); {
					data := it.Leaf()
					err := binary.Read(bytes.NewBuffer(data), binary.LittleEndian, &ct) // Reads from data stores in ct
					if err != nil {
					...
					// 		podEnergy[podName].CGroupPID = ct.CGroupPID   //getting it from ebpf collector
					// 		podEnergy[podName].PID = ct.PID               //getting it from ebpf collector
					// 		podEnergy[podName].Command = C.GoString(comm) //getting it from ebpf collector
					// 	}
					...
					// 	val := ct.CPUCycles //getting it from ebpf collector
					// 	podEnergy[podName].CurrCPUCycles += val
					// 	podEnergy[podName].AggCPUCycles += val
					// 	aggCPUCycles += val
					// 	val = ct.CPUInstr //getting it from ebpf collector
					// 	podEnergy[podName].CurrCPUInstr += val
					// 	podEnergy[podName].AggCPUInstr += val
					// 	aggCPUInstr += val
					// 	val = ct.CacheMisses //getting it from ebpf collector
					...
					// 	}
					// 	rBytes, wBytes, disks, err := pod_lister.ReadCgroupIOStat(ct.CGroupPID) //getting it from ebpf collector
					...
					// 		if _, ok := cgroupIO[ct.CGroupPID]; !ok { //getting it from ebpf collector
